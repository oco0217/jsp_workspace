					...
					case "newFile":
						...
						if (item.getSize() > 0) {
							...
							if (old_file != null) {
								...
								int isOk = fileHandler.deleteFile(savePath, old_file);
								log.info("수정되기전 사진 삭제" + (isOk>0?"성공!!" : "실패 ㅠㅠ"));
		...
		case "delete":
			try {
				...
				if (imageFileName != null) {
					...
					FileRemoveHandler fileHandler = new FileRemoveHandler();
					isDel = fileHandler.deleteFile(savePath, imageFileName);

				}
				// bno 댓글 삭제 요청 => serviceimple에서 cdao요청
